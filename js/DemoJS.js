...
// Grabs the year from
function extractYear(sql) { return sql.split("_")[1]; }

// query string: ?foo=lorem&bar=&baz  //var foo = getParameterByName('foo'); ->
// "lorem" //have the query string to have like: ?control=TagOption1&control
...
$(document).ready(function() {
  ...
  function twoRadioForms(S) {
    ...
    for (i in demo) {
      // console.log(demo[i])
      var listItem = $('<li/>'), input = $('<input/>'), label = $('<label/>');
      listItem.attr("class", "layer");
      input.attr({"type" : "radio", "id" : demo[i], "name" : S});
  ...
  function initializeMaps() {
    ...
    LR.forEach(function(currentLR) {
      ...
      $("input[name='" + currentLR + "']")
          .change(function() { clearandReload(); });
    });
    loadSubLayers()
    window.setTimeout(addInteractivity, 1000);
  }
  map.on('move', clip);
  ...
  function clearSubLayers() {
    Object.keys(layerSides).forEach(function(currentLayer) {
      layerSides[currentLayer].getSubLayers().forEach(function(
          sublayer) { sublayer.remove(); });
    });
  }
  ...
  function addInteractivity() {
    ...
    subArray.forEach(function(currentLayer) {
      ...
      currentLayer.on('featureClick', function(event, latlng, pos, data,
                                               layerIndex) {
                                                 console.log(data)
        pointClicked({youtube_id : data.youtube_id, yt_start : data.yt_start});
  ...
  function loadSubLayers() {
    ...
    Object.keys(layerSides).forEach(function(thisLayer) {
      $.each($("input[name='" + layerShortHand[thisLayer] + "']:checked"),
             function() {
               var demo = layerSides[thisLayer].createSubLayer(
                   layers.demographics[$(this).attr("id")]);
               console.log(layers.Points[$(this).attr("id")])
               layerSides[thisLayer].createSubLayer(
                   layers.Points[$(this).attr("id")]);
               demo.setInteraction(true);
               callOutToInfoWindow.push(demo);
               demo.on('featureClick', function(event, latlng, pos, data,
                                                layerIndex) {
                 ...
                 if (OverclipX === true) {
                   layerDecade =
                       layerSides["rightLayer"].layers[0].options.sql.substring(
                           8, 12);
                 } else {
                   layerDecade =
                       layerSides["leftLayer"].layers[0].options.sql.substring(
                           8, 12);
                 }
                 console.log(data);
                 if (data.year == parseInt(layerDecade)) {
                   ...
                   console.log("here");
                   console.log(pieChartData["0"].value = data.hispanic);
